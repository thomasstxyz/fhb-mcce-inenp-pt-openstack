...
# Create floating ip
resource "openstack_networking_floatingip_v2" "floatingip_kube_node" {
  count = var.kube_node_count
  pool = data.openstack_networking_network_v2.provider_network.name
}

# Attach floating ip to instance
resource "openstack_compute_floatingip_associate_v2" "floatingip_kube_node" {
  count       = var.kube_node_count
  floating_ip = openstack_networking_floatingip_v2.floatingip_kube_node[count.index].address
